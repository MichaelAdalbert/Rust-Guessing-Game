...
impl Application for GuessingGame {
    ...
    fn title(&self) -> String {
        format!("Guessing Game {} {}", if self.guessing_output.len() == 0 { "" } else { "-" }, self.guessing_output)
    }
    ...
    fn view(&self) -> Element<'_, Self::Message, Renderer<Self::Theme>> {
        ...
        let input = text_input("Enter a number ?", self.guessing_input.as_str()).on_input(GuessingGameMessage::Input);
